...
```java
 ...
 실제 프로그램에서 잘 발생하는 오류들에 대해서 알아보자.
...
a[3]은 a 배열의 4번째 값이므로 a 배열에서 구할 수 없는 값이다. 그래서 ArrayIndexOutOfBoundsException 오류가 발생했다.  이와같은 예외가 발생하면 프로그램을 중단하고 오류메시지를 보여준다.
...
프로그램 수행 도중 예외가 발생하면 프로그램이 오류에 의해 중지되거나 예외 처리에 의해 catch 구문이 실행된다. 하지만 어떤 예외가 발생하더라도 반드시 실행되어야 하는 부분이 있어야 한다면 어떻게 해야 할까?
다음의 예제를 보도록 하자.
public class Sample {
...
sample.shouldBeRun() 메서드는 반드시 실행되어야 하는 메서드라고 가정해 보자. 이런 경우를 처리하기 위해 자바는 finally 구문을 제공한다.
다음의 예를 보자.
public class Sample {
    public void shouldBeRun() {
        System.out.println("ok thanks.");
...
finally 구문은 try 문장 수행 중 예외발생 여부에 상관없이 무조건 실행된다. 따라서 위 코드를 실행하면 sample.shouldBeRun() 메서드가 수행되어 "ok, thanks" 문장이 출력될 것이다.
...
sayNick 메서드 뒷부분에 throws 라는 구문을 이용하여 FoolException을 위로 보낼 수가 있다. ("예외를 뒤로 미루기"라고도 한다.) 이와 같이 sayNick 메서드를 변경하면 이제 main 메서드에서 컴파일 에러가 발생할 것이다. throws 구문 때문에 FoolException의 예외를 처리해야 하는 대상이 sayNick 메서드에서 main 메서드(sayNick 메서드를 호출하는 메서드)로 변경되었기 때문이다.
...
이렇게 각각의 메서드에 예외가 처리되어 있다면 포장은 되었는데 발송은 안되고 포장도 안되었는데 발송이 되고 이런 상황이 연출될 것이다. 실제 프로젝트에서도 두번째 경우처럼 트랜잭션관리를 잘못하여 고생하는 경우를 많이 보았는데 이것은 일종의 재앙에 가깝다.
