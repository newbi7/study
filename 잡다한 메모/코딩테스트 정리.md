...
str.split(" ");
str.substring(0, 5);
substring은 문자열에서 일부 문자열을 추출하는 메서드입니다. substring(startIndex)은 startIndex 인덱스부터 문자열의 끝까지의 부분문자열을 반환하며, substri(startIndex, endIndex)은 startIndex 인덱스부터 endIndex 바로 전 인덱스까지의 부분문자열을 반환합니다.
