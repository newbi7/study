...
이 코드는 Spring MVC를 사용하여 웹 어플리케이션의 홈 페이지를 처리하는 Controller 클래스입니다.
...
EL(Expression Language)은 JSP(JavaServer Pages)에서 사용되는 표현 언어입니다. EL은 JSP 페이지에서 Java 코드를 간단하게 사용할 수 있도록 하는 언어로, JSP에서 사용되는 변수, 메소드, 표현식 등을 처리하는 데 사용됩니다.

EL은 ${}와 #{} 두 가지 표현식으로 사용됩니다. ${}는 값을 출력하는 데 사용되며, #{}는 값을 설정하는 데 사용됩니다. EL을 사용하면 JSP 페이지에서 다음과 같은 작업을 할 수 있습니다.

변수 출력: EL을 사용하여 변수 값을 출력할 수 있습니다. 예를 들어, ${name}은 "name"이라는 변수의 값을 출력합니다.
수식 계산: EL을 사용하여 수식을 계산할 수 있습니다. 예를 들어, ${1 + 2}는 3을 출력합니다.
객체 접근: EL을 사용하여 객체의 속성에 접근할 수 있습니다. 예를 들어, ${user.name}은 "user" 객체의 "name" 속성 값을 출력합니다.
컬렉션 접근: EL을 사용하여 컬렉션 객체의 원소에 접근할 수 있습니다. 예를 들어, ${list[0]}은 "list"라는 리스트 객체의 첫 번째 원소 값을 출력합니다.
메소드 호출: EL을 사용하여 메소드를 호출할 수 있습니다. 예를 들어, ${user.getName()}은 "user" 객체의 getName() 메소드를 호출하여 반환된 값을 출력합니다.
EL은 JSP의 생산성을 높여주는 기능 중 하나입니다. EL을 사용하면 JSP 페이지에서 Java 코드를 더욱 간단하게 사용할 수 있으며, 코드의 가독성과 유지 보수성을 높일 수 있습니다.
```
...
Spring MVC는 MVC(모델-뷰-컨트롤러) 아키텍처 패턴을 사용하여 웹 애플리케이션을 구성하고 구조화하는 웹 프레임워크입니다. 클라이언트가 Spring MVC 웹 애플리케이션에 요청을 보내면 DispatcherServlet은 요청을 수신하고 일련의 단계에 따라 처리합니다.

스프링 MVC에서 요청을 처리하는 단계는 다음과 같습니다.

DispatcherServlet : DispatcherServlet은 Spring MVC 웹 응용 프로그램의 전면 컨트롤러입니다. 들어오는 모든 요청을 수신하여 적절한 handler에 위임합니다.

handler mapping : 요청이 수신되면 DispatcherServlet은 handler mapping을 참조하여 요청을 처리해야 하는 컨트롤러를 결정합니다. HandlerMapping은 요청을 특정 처리기 또는 컨트롤러에 매핑하는 작업을 담당합니다.

handler Adapter: handlermapping이 요청을 처리해야 하는 컨트롤러를 결정하면 DispatcherServlet은 적절한 controller를 호출합니다. HandlerAdapter는 DispatcherServlet의 실행 모델에 컨트롤러를 적용하는 작업을 담당합니다.

controller: 컨트롤러는 요청을 처리하고 모델 데이터 및 뷰 이름이 포함된 ModelAndView 개체를 반환합니다.

model: 모델은 뷰에서 응답을 렌더링하는 데 사용할 데이터를 보유하는 데이터 구조입니다. 컨트롤러는 요청과 관련된 데이터로 모델을 채웁니다.

viewResolver: ViewResolver는 컨트롤러가 반환한 뷰 이름을 실제 View 개체로 확인하는 역할을 합니다. View 개체는 응답 렌더링을 담당합니다.

model: model은 응답 렌더링을 담당합니다. 컨트롤러에서 모델 데이터를 수신하고 이를 사용하여 응답을 생성합니다.
...
ㅁ Servlet -> response.sendRedirect()

controller -> new ModelAndView(O,O) -> new RedirectView();

ㅁ MarshallingView->XML
ㅁ MappingJacksonJsonViewClass -> JSON
